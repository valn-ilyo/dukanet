...
const loginWithGoogle = async () => {
  const { error } = await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: {
      redirectTo: `${window.location.origin}/dukanet/`,
    },
  })
...
<template>
  <v-container :max-width="400" class="d-flex flex-column align-center justify-center">
    ...
    <v-card-subtitle class="pb-4 text-subtitle-2 text-wrap text-center text-onBackground">
      Imagine finding exactly what you need, or reaching customers, just down the street. This
      modern platform is built for you to connect, trade, and grow effortlessly with businesses and
      customers in your immediate community.
    </v-card-subtitle>
